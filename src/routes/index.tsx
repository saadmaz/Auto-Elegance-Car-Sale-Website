...
import { CinematicDrive } from "@/components/CinematicDrive";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Hand-picked European luxury cars, sourced, verified, and delivered to your door. Concierge auto-buying for collectors and enthusiasts." },
      { property: "og:title", content: "MAISON AUTO — Curated Luxury Cars" },
      { property: "og:description", content: "Concierge sourcing of European luxury cars. Verified. Delivered." },
    ],
  }),
  component: Index,
});
...
const COLLECTION = [
  { name: "Porsche 911 Carrera", year: "2023", price: "€148,000", km: "8,200 km", img: car1, tag: "Silver Pearl" },
  { name: "BMW M4 Competition", year: "2024", price: "€132,500", km: "3,400 km", img: car2, tag: "Tanzanite Blue" },
  { name: "Audi RS7 Sportback", year: "2023", price: "€189,000", km: "12,100 km", img: car3, tag: "Daytona Grey" },
];
    ...
    <header className="fixed top-0 z-50 w-full">
      <div className="mx-auto flex max-w-[1600px] items-center justify-between px-6 py-5 md:px-12">
        ...
        <nav className="hidden items-center gap-10 md:flex">
          {NAV.map((n) => (
            <a key={n.href} href={n.href} className="group relative text-sm tracking-wide text-foreground/70 transition hover:text-foreground">
              {n.label}
              <span className="absolute -bottom-1 left-0 h-px w-0 bg-gold transition-all group-hover:w-full" />
            </a>
          ))}
        </nav>
        <a href="#contact" className="group relative inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/5 px-5 py-2.5 text-sm text-foreground backdrop-blur transition hover:bg-gold/10">
...
function Hero() {
  ...
  return (
    <section ref={ref} className="relative min-h-[120vh] overflow-hidden hero-gradient grain">
      {/* Background grid */}
      ...
      {/* Floating 3D emblem */}
      <motion.div style={{ y }} className="absolute right-[6%] top-[18%] hidden h-[380px] w-[380px] lg:block">
        <EmblemScene />
      </motion.div>

      <div className="relative mx-auto flex min-h-screen max-w-[1600px] flex-col justify-center px-6 pt-32 md:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className="flex items-center gap-3 text-xs tracking-[0.4em] text-gold/80">
        ...
        <div className="mt-12 grid gap-12 md:grid-cols-2 md:items-end">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6, duration: 1 }} className="max-w-md text-base leading-relaxed text-muted-foreground md:text-lg">
            ...
            handed over with the keys, by people who care.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 1 }} className="flex flex-wrap items-center gap-4">
            <a href="#collection" className="group relative inline-flex items-center gap-3 overflow-hidden rounded-full bg-gold px-8 py-4 text-sm font-medium tracking-wide text-primary-foreground transition hover:scale-[1.02]">
              <span className="relative z-10">View the collection</span>
              <svg className="relative z-10 h-4 w-4 transition group-hover:translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 5l7 7-7 7"/></svg>
            ...
            <a href="#contact" className="inline-flex items-center gap-2 rounded-full border border-foreground/20 px-8 py-4 text-sm tracking-wide transition hover:border-gold hover:text-gold">
              Request a quote
            </a>
          </motion.div>
        </div>
        ...
        {/* Stats strip */}
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 1 }} className="relative z-10 mt-auto grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border/40 md:grid-cols-4">
          {[
            ...
            <div key={s.v} className="bg-background/80 px-6 py-6 backdrop-blur">
              ...
              <div className="mt-1 text-xs tracking-widest text-muted-foreground">{s.v.toUpperCase()}</div>
            </div>
          ))}
        </motion.div>
      </div>
...
function Marquee() {
  const words = ["Porsche", "Mercedes-AMG", "BMW M", "Audi RS", "Lamborghini", "Ferrari", "Aston Martin", "Bentley", "McLaren", "Maserati"];
  return (
    <div className="relative overflow-hidden border-y border-border bg-card py-8">
      <div className="marquee flex w-max gap-16">
        {[...words, ...words].map((w, i) => (
          <div key={i} className="flex items-center gap-16 font-display text-5xl text-foreground/30 md:text-7xl">
            {w}
...
function Atelier() {
  ...
  return (
    <section id="atelier" ref={ref} className="relative overflow-hidden py-32 md:py-48">
      <div className="mx-auto grid max-w-[1600px] gap-16 px-6 md:grid-cols-12 md:px-12">
        <div className="md:col-span-5 md:sticky md:top-32 md:self-start">
          ...
          <p className="mt-8 max-w-md text-muted-foreground">
            For over a decade we've made the unromantic act of buying a car feel
            like coming home. Every car we touch is sourced personally, verified
            against a 32-point protocol, and prepared with reverence.
          </p>
        ...
        <div className="relative md:col-span-7">
          <motion.div style={{ y, rotate }} className="relative aspect-[4/5] overflow-hidden rounded-3xl border border-border">
            <img src={carTop} alt="Aerial luxury car" className="h-full w-full object-cover" loading="lazy" width={1920} height={1280} />
...
function Process() {
  const steps = [
    { n: "01", t: "We search", d: "You tell us the dream. We crawl auctions, private dealers, and our European network — surfacing only what fits." },
    { n: "02", t: "We verify", d: "Each candidate passes a 32-point inspection, full history check, and an in-person review by our specialists." },
    { n: "03", t: "We deliver", d: "Paperwork, transport, registration — handled. The car arrives at your door, fueled, detailed, and ready." },
  ];
  return (
      ...
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="mt-6 font-display text-6xl leading-[1] tracking-tight md:text-7xl">
              Three steps. <br/><span className="italic text-foreground/50">Zero compromise.</span>
            </h2>
          </div>
          <p className="max-w-sm text-muted-foreground">
            Most clients receive their car within 14–28 days. Some have waited months for the right one. We don't rush perfection.
          </p>
        </div>
        ...
        <div className="mt-20 grid gap-6 md:grid-cols-3">
          {steps.map((s, i) => (
            ...
            >
              <div className="font-display text-7xl text-gold/30 transition group-hover:text-gold/60">{s.n}</div>
...
function Collection() {
  return (
    <section id="collection" className="relative overflow-hidden py-32 md:py-48">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        <div className="flex items-end justify-between">
          <div>
            <div className="text-xs tracking-[0.4em] text-gold/80">— COLLECTION</div>
            <h2 className="mt-6 font-display text-6xl leading-[1] tracking-tight md:text-7xl">In the garage <span className="italic gold-shine">now</span>.</h2>
          </div>
          <a href="#" className="hidden text-sm text-muted-foreground hover:text-gold md:inline">View all 42 cars →</a>
        </div>
          ...
          {COLLECTION.map((car, i) => (
            ...
            >
              ...
              <div>
                ...
                <h3 className="mt-6 font-display text-5xl leading-[1.05] md:text-6xl">{car.name}</h3>
                <div className="mt-8 grid grid-cols-3 gap-px overflow-hidden rounded-xl border border-border bg-border">
                  {[
                    ...
                    <div key={d.v} className="bg-card px-4 py-4">
                      <div className="text-xs tracking-widest text-muted-foreground">{d.v.toUpperCase()}</div>
                      <div className="mt-1 font-display text-xl">{d.k}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-8 flex gap-4">
                  <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3 text-sm text-primary-foreground transition hover:scale-[1.02]">
                    Reserve this car →
                  </a>
                  <a href="#" className="inline-flex items-center gap-2 rounded-full border border-foreground/20 px-6 py-3 text-sm transition hover:border-gold hover:text-gold">
                    Full spec
                  </a>
                </div>
              </div>
...
function Voices() {
  const quotes = [
    { q: "They found the exact spec I'd been hunting for two years. Delivered in three weeks. Flawless.", who: "Mateusz K.", car: "Porsche Cayman GT4" },
    { q: "I never set foot in a dealership. The car arrived better than described — and that's saying something.", who: "Agnieszka W.", car: "Audi e-tron GT" },
    { q: "Worth every euro. The peace of mind alone is the product.", who: "Krzysztof B.", car: "BMW M2 CS" },
  ];
  return (
    <section id="voices" className="relative overflow-hidden border-y border-border bg-card py-32 md:py-48">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
          ...
          <div className="grid gap-6">
            {[
              ...
              <label key={f.l} className="block">
                <span className="text-xs tracking-widest text-muted-foreground">{f.l.toUpperCase()}</span>
                <input
                  type={f.t}
            ...
            <label className="block">
              ...
              <textarea rows={3} className="mt-2 w-full resize-none border-b border-border bg-transparent py-3 text-lg outline-none transition focus:border-gold" />
            </label>
      ...
      <div className="mx-auto max-w-[1600px] px-6 py-16 md:px-12">
        <div className="flex flex-col items-start justify-between gap-12 md:flex-row">
          <div>
            ...
            <div className="mt-2 text-xs tracking-[0.3em] text-muted-foreground">CURATED CARS · DELIVERED WITH CARE</div>
          </div>
          <div className="flex flex-wrap gap-12 text-sm">
            <div>
              <div className="mb-4 text-xs tracking-widest text-gold">EXPLORE</div>
              {["Collection", "Process", "Atelier", "Journal"].map((x) => (
                <a key={x} href="#" className="block py-1 text-muted-foreground hover:text-foreground">{x}</a>
              ))}
            </div>
            <div>
